...
class _SchedulePageState extends State<SchedulePage> {
  ...
  Future<void> _initDbAndLoad() async {
    ...
    // Insert test data (conflicts ignored for hot reload)
    try {
      ...
      // Locations (inserts ignore duplicates)
      final int locSci2460 = await _dbp.insertLocation(Location(buildingName: 'SCI', room: '2460'));
      final int locEng120 = await _dbp.insertLocation(Location(buildingName: 'ENG', room: '120'));
      final int locSci1502 = await _dbp.insertLocation(Location(buildingName: 'SCI', room: '1502'));
      final int locSci110 = await _dbp.insertLocation(Location(buildingName: 'SCI', room: '110'));
      final int locEng220 = await _dbp.insertLocation(Location(buildingName: 'ENG', room: '220'));

      // Sessions (course_code, type, section, startMin, endMin, locationId, weekdays 0=Mon..6=Sun)
      ...
      // Assignments (choose due dates on intended weekdays in 2025)
    ...
    // Build DaySchedule objects reading back from DB
    final List<String> weekdayNames = [
    'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'
    ];

    // Query sessions for each weekday
    final Map<int, List<Session>> classesByDay = {};
    for (int wd = 0; wd < 7; wd++) {
    final List<Session> sessions = await _dbp.getSessionsByWeekday(wd);
    classesByDay[wd] = sessions;
    }

    // Query all assignments and group by weekday of due date
    final Database db = await _dbp.database;
    final List<Map<String, Object?>> rows = await db.query('assignments');
    final Map<int, List<Assignment>> assignmentsByDay = {};
    for (final Map<String, Object?> r in rows) {
    final Assignment a = Assignment.fromMap(r);
    DateTime dt;
    try {
    ...
    final int wd = dt.weekday - 1; // DateTime.weekday: 1=Mon..7=Sun -> 0..6
    assignmentsByDay.putIfAbsent(wd, () => <Assignment>[]);
    assignmentsByDay[wd]!.add(a);
    }

    // Build final list for weekdays with content
    final List<DaySchedule> built = [];
    for (int wd = 0; wd <= 6; wd++) {
    final List<Session> classes = classesByDay[wd] ?? <Session>[];
    final List<Assignment> assigns = assignmentsByDay[wd] ?? <Assignment>[];
    if (classes.isEmpty && assigns.isEmpty) continue; // skip empty days
    built.add(DaySchedule(
  ...
  void _showSimpleDialog(BuildContext context) {
    showDialog<String>(
      ...
      if (value == 'Add Class') {
        // Class form dialog
        final TextEditingController courseCodeCtl = TextEditingController();
        final TextEditingController courseTitleCtl = TextEditingController();
        final TextEditingController sectionCtl = TextEditingController();
        final TextEditingController buildingCtl = TextEditingController();
          ...
          builder: (BuildContext ctx) {
            return StatefulBuilder(builder: (BuildContext ctx, StateSetter setState) {
              Future<void> pickStart() async {
                final TimeOfDay? t = await showTimePicker(context: ctx, initialTime: startTime!);
                if (t != null) {
              ...
              Future<void> pickEnd() async {
                final TimeOfDay? t = await showTimePicker(context: ctx, initialTime: endTime!);
                if (t != null) {
                  setState(() => endTime = t);
                }
              }
                        ...
                        children: <Widget>[
                          TextFormField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Course Code', hintText: 'e.g. CSCI101'),
                            validator: (String? v) =>
                            (v == null || v.trim().isEmpty) ? 'Course code required' : null,
                          ),
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            onChanged: (String? v) => setState(() => typeValue = v ?? 'lecture'),
                          ...
                          TextFormField(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Section (optional)', hintText: 'e.g. A, W1'),
                          ),
                          const SizedBox(height: 8),
                          Row(
                            children: <Widget>[
                          ...
                          Wrap(
                            ...
                            children: List<Widget>.generate(7, (int i) {
                              ...
                              return FilterChip(
                                ...
                                onSelected: (bool sel) =>
                                    setState(() => weekdaySelected[i] = sel),
                              );
                            }),
                          ),
                              ...
                              Expanded(
                                child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    final int sMinutes = toMinutes(startTime!);
                                    final int eMinutes = toMinutes(endTime!);
                                    ...
                                    final String courseCode = courseCodeCtl.text.trim();
                                    final String? courseTitle = courseTitleCtl.text.trim().isEmpty
                                        ? null
                                        : courseTitleCtl.text.trim();
                                    final String? section =
                                    sectionCtl.text.trim().isEmpty ? null : sectionCtl.text.trim();
                                    final String? building =
                                    buildingCtl.text.trim().isEmpty ? null : buildingCtl.text.trim();
                                    final String? room =
                                    roomCtl.text.trim().isEmpty ? null : roomCtl.text.trim();

                                    try {
                                      // Ensure course exists (replace semantics)
                                      await _dbp.insertCourse(
                                          Course(courseCode: courseCode, title: courseTitle));

                                      // Insert location if provided and get id
                                      int? locId;
                                      if (building != null) {
                                        locId = await _dbp.insertLocation(
                                            Location(buildingName: building, room: room));
                                      }
        ...
        final TextEditingController submissionCtl = TextEditingController();
          ...
          builder: (BuildContext ctx) {
            return StatefulBuilder(builder: (BuildContext ctx, StateSetter setState) {
              ...
              Future<void> pickTime() async {
                final TimeOfDay? t = await showTimePicker(context: ctx, initialTime: dueTime);
                if (t != null) {
                  setState(() => dueTime = t);
                }
              }
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          TextFormField(
                            ...
                            decoration:
                            const InputDecoration(labelText: 'Submission Location (optional)'),
                          ),
                          const SizedBox(height: 8),
                          Row(
                            children: <Widget>[
                              Expanded(
                                child: OutlinedButton(
                                  onPressed: pickDate,
                                  child: Text(
                                      'Date: ${dueDate.toLocal().toIso8601String().substring(0, 10)}'),
                                ),
                              ),
                              const SizedBox(width: 8),
                              Expanded(
                                child: OutlinedButton(
                                  onPressed: pickTime,
                                  child: Text('Time: ${dueTime.format(ctx)}'),
                                ),
                              ),
                            ],
                          ),
                              ...
                              Expanded(
                                child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    final String courseCode = courseCodeCtl.text.trim();
                                    final String title = titleCtl.text.trim();
                                    final String? submission = submissionCtl.text
                                        .trim()
                                        .isEmpty
                                        ? null
                                        : submissionCtl.text.trim();
                                    ...
                                    final DateTime combined = DateTime(
                                        ...
                                        dueTime.hour,
                                        dueTime.minute);
                                    final String iso = isoFromDateTime(combined);

                                    try {
                                      // Ensure course exists
                                      await _dbp.insertCourse(
                                          Course(courseCode: courseCode, title: null));

                                      final Assignment a = Assignment(
                                          courseCode: courseCode,
                                          title: title,
                                          dueDatetime: iso,
                                          submissionLocation: submission);
      ...
      child: ExpansionTile(
        ...
        title: Text(
          ...
          style: base.titleLarge
              ?.copyWith(color: const Color(0xFFF7ECE1), fontWeight: FontWeight.w600),
        ),
        subtitle: Text(
      ...
      child: Column(
        ...
        children: <Widget>[
          Row(children: <Widget>[
            ...
            const SizedBox(width: 8),
            Text(label,
                style: base.titleMedium?.copyWith(color: const Color(0xFF759FBC))),
          ]),
          const SizedBox(height: 8),
          ...sessions.map((Session s) {
            final String title = s.courseTitle ?? s.courseCode;
            final String timeRange =
            minutesTo12HourRange(s.startMin, s.endMin);
            final String loc = (s.locationBuilding != null && s.locationBuilding!.isNotEmpty)
                ? ((s.locationRoom == null || s.locationRoom!.isEmpty)
                ? s.locationBuilding!
                : '${s.locationBuilding!} ${s.locationRoom!}')
                : '';
            return GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    const Icon(Icons.schedule_outlined, color: Color(0xFF9FB3C6)),
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          ...items.map((Assignment a) {
            ...
            final String pretty = 'Due: ${_formatMonthDay(dt)}';
            return GestureDetector(
              onDoubleTap: () {
                showDialog<void>(
                  context: context,
              ...
              child: Container(
                ...
                child: Row(
                  children: <Widget>[
                    const Icon(Icons.check_circle_outline, color: Color(0xFF9FB3C6)),
                    const SizedBox(width: 10),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: <Widget>[
                          Text(a.title,
                              style: base.bodyLarge
                                  ?.copyWith(color: const Color(0xFFF7ECE1), fontWeight: FontWeight.w600)),
                          const SizedBox(height: 2),
                          Text('$pretty  •  ${a.submissionLocation ?? ''}',
                              style: base.bodyMedium?.copyWith(color: const Color(0xFF9FB3C6))),
                        ],
                      ),
                    ),
...
// --- SQLite schema and helpers for courses, sessions, session_days, locations, assignments ---
...
class Course {
  ...
  static Course fromMap(Map<String, Object?> m) => Course(
...
class Location {
  ...
  static Location fromMap(Map<String, Object?> m) => Location(
    id: m['id'] as int?,
  ...
  String displayName() =>
      (room == null || room!.isEmpty) ? buildingName : '$buildingName ${room!}';
}
...
class Assignment {
  ...
  final String courseCode;
  final String title;
  final String dueDatetime; // 'YYYY-MM-DD HH:MM'
  ...
  static Assignment fromMap(Map<String, Object?> m) => Assignment(
    id: m['id'] as int?,
    courseCode: m['course_code'] as String,
    title: m['title'] as String,
...
class Session {
  ...
  final String? courseTitle; // populated when joined with courses
  ...
  static Session fromMap(Map<String, Object?> m, List<int> days) => Session(
    id: m['id'] as int?,
    courseCode: m['course_code'] as String,
...
class DatabaseProvider {
  ...
  Future<Database> _initDB() async {
    ...
    final String path = p.join(dbPath, 'schedule.db');
  ...
  Future<void> _onCreate(Database db, int version) async {
    ...
    await db.execute(
        'CREATE INDEX idx_session_days_weekday ON session_days(weekday);');
    await db.execute(
  ...
  Future<Course?> getCourse(String courseCode) async {
    ...
    await db.query('courses', where: 'course_code = ?', whereArgs: [courseCode]);
  ...
  Future<Location?> getLocationByBuildingAndRoom(String building, String? room) async {
    ...
    final List<Map<String, Object?>> res = await db.query(
      'locations',
      where: 'building_name = ? AND (room IS ? OR room = ?)',
      whereArgs: [building, room, room],
    );
    if (res.isEmpty) return null;
    return Location.fromMap(res.first);
  }

  Future<int> _ensureLocationInTxnWithBuilding(
      Transaction txn, String building, String? room) async {
  ...
  // Sessions (inserts session + its days)
  Future<int> insertSession(Session s) async {
    ...
    return await db.transaction<int>((Transaction txn) async {
      ...
      if (locId == null && s.locationBuilding != null && s.locationBuilding!.isNotEmpty) {
        locId = await _ensureLocationInTxnWithBuilding(
            txn, s.locationBuilding!, s.locationRoom);
      }

      final Map<String, Object?> sessionMap = s.toMap();
      sessionMap['location_id'] = locId;
      final int sessionId = await txn.insert(
          'sessions', sessionMap,
          conflictAlgorithm: ConflictAlgorithm.abort);

      for (final int d in s.weekdays) {
        await txn.insert('session_days', {'session_id': sessionId, 'weekday': d},
  ...
  Future<void> updateSession(Session s) async {
    ...
    await db.transaction((Transaction txn) async {
      ...
      for (final int d in s.weekdays) {
        await txn.insert('session_days',
            {'session_id': s.id, 'weekday': d},
            conflictAlgorithm: ConflictAlgorithm.replace);
      }
    });
  }
  ...
  Future<void> updateAssignment(Assignment a) async {
    if (a.id == null) {
      throw ArgumentError('Assignment.id required for update');
    }
    final Database db = await database;
  ...
  Future<List<Assignment>> getAssignmentsForCourse(String courseCode) async {
    final Database db = await database;
    final List<Map<String, Object?>> rows = await db.query('assignments',
        where: 'course_code = ?', whereArgs: [courseCode], orderBy: 'due_datetime');
    return rows.map((r) => Assignment.fromMap(r)).toList();
  }

  // Queries for sessions with building+room included
  Future<List<Session>> getSessionsByWeekday(int weekday) async {
    ...
    final List<Session> out = <Session>[];
    for (final Map<String, Object?> row in rows) {
      final int sessionId = row['id'] as int;
      final List<Map<String, Object?>> dayRows =
      await db.query('session_days', where: 'session_id = ?', whereArgs: [sessionId]);
      final List<int> days = dayRows.map((r) => r['weekday'] as int).toList();
  ...
  Future<List<Session>> getAllSessions() async {
    final Database db = await database;
    final List<Map<String, Object?>> rows = await db.rawQuery(
        'SELECT s.*, l.building_name, l.room, c.title AS course_title '
            'FROM sessions s '
            'LEFT JOIN locations l ON s.location_id = l.id '
            'LEFT JOIN courses c ON s.course_code = c.course_code '
            'ORDER BY s.course_code, s.start_min');
    final List<Session> out = <Session>[];
    for (final Map<String, Object?> row in rows) {
      final int sessionId = row['id'] as int;
      final List<Map<String, Object?>> dayRows =
  ...
  Future<List<Location>> getAllLocations() async {
    ...
    await db.query('locations', orderBy: 'building_name, room');
...
int hhmmToMinutes(String hhmm) {
  ...
  if (parts.length != 2) {
    throw FormatException('Time must be HH:MM');
  }
...
String minutesTo12HourRange(int start, int end) =>
    '${_format12Hour(start)}–${_format12Hour(end)}';
